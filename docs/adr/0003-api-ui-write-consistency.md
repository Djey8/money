...
- This does not fully solve concurrent-write races between two _simultaneous_ agent writes to the same array-based entity; the per-item CouchDB write should use its own retry-on-409 logic (already proven in `backend/routes/data.js`'s batch-write retry logic) to serialize those safely at the document level.

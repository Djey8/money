...
Kept faithful to the original: the average is computed over *all* historical months with expense data, not a fixed trailing window (e.g. the last 12 months), and the current calendar month is always excluded since it's necessarily incomplete.

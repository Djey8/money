...
describe('POST /api/logs/frontend', () => {
  ...
  skipIf(!dbAvailable, 'accepts a valid batch of log entries', async () => {
    const res = await authed('post', '/api/logs/frontend').send({
      logs: [
        {
  ...
  skipIf(!dbAvailable, 'accepts an empty log entry with defaults', async () => {
    const res = await authed('post', '/api/logs/frontend').send({
      logs: [{ timestamp: new Date().toISOString(), action: 'minimal' }],
    });
  ...
  skipIf(!dbAvailable, 'returns 400 when logs is not an array', async () => {
    const res = await authed('post', '/api/logs/frontend').send({ logs: 'not-an-array' });
  ...
  skipIf(!dbAvailable, 'returns 400 when logs key is missing', async () => {
    const res = await authed('post', '/api/logs/frontend').send({ something: 'else' });
  ...
  it('returns 401 without authentication', async () => {
    const res = await request(app).post('/api/logs/frontend').send({ logs: [] });

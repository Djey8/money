...
describe('Batch write stress tests', () => {
  ...
  skipIf(
    !dbAvailable,
    'handles 50+ writes in a single batch',
    async () => {
  ...
  skipIf(
    !dbAvailable,
    'handles concurrent batch writes from the same user',
    async () => {
      const batch1 = authed('post', '/api/data/write/batch').send({
      ...
      const batch2 = authed('post', '/api/data/write/batch').send({
        writes: Array.from({ length: 10 }, (_, i) => ({
  ...
  skipIf(!dbAvailable, 'rejects batch where a write has no path', async () => {
    const res = await authed('post', '/api/data/write/batch').send({
      writes: [{ path: 'valid/path', data: 1 }, { data: 'missing-path' }],
    });

...
- **Root `package.json`'s `@types/crypto-js`/`@types/jest` devDependencies look redundant with `packages/domain/package.json`'s own — they aren't, don't remove them.** A workspace member's devDependency that root doesn't _also_ independently require gets silently skipped by `npm ci` when it runs while that workspace member's own directory/`package.json` is entirely absent from the working tree — confirmed empirically (a `--no-cache` container build reproduces it; a normal `npm ci` after a full checkout, e.g. in CI, does not, since every workspace's `package.json` already exists on disk by then). This bites specifically in `Dockerfile`'s pattern of `COPY package*.json ./` then `npm ci` _before_ `COPY . .` brings in `packages/domain/` — declaring the same package at root works around it. If you add a new `packages/domain` (or `apps/mcp`) devDependency that isn't already satisfied by something root itself needs, check whether it survives a `--no-cache` Docker build (not just local `npm install`, which a long-lived dev `node_modules` would mask) before assuming it's installed.

...
export function createExplorativeAnalytics(container: any) {
  ...
  contentDiv.append('div').attr('id', 'explorative-filter-placeholder');

  // Period Comparison Toggle
  createPeriodComparisonToggle(contentDiv);

  // Placeholder for visualization (will be filled by update)
  contentDiv.append('div').attr('id', 'explorative-viz-placeholder');

  // Placeholder for transaction table (will be filled by update)
  contentDiv.append('div').attr('id', 'explorative-table-placeholder');
...
export function createBreadcrumbNavigation(container: any) {
  ...
  StatsComponent.explorativeBreadcrumbs.forEach((crumb, index) => {
    ...
    breadcrumbDiv
      ...
      .style(
        'color',
        index === StatsComponent.explorativeBreadcrumbs.length - 1
          ? 'var(--color-text)'
          : '#1976d2',
      )
      .style(
        'font-weight',
        index === StatsComponent.explorativeBreadcrumbs.length - 1 ? 'bold' : 'normal',
      )
      ...
      .on('click', (event) => {
        ...
        StatsComponent.explorativeBreadcrumbs = StatsComponent.explorativeBreadcrumbs.slice(
          0,
          index + 1,
        );
        if (crumb.level === StatsComponent.translateService.instant('BI.breadcrumbAccount')) {
          ...
          crumb.level === StatsComponent.translateService.instant('BI.breadcrumbCategory')
        ) {
...
export function getExplorativeFilteredTransactions(): any[] {
  ...
  transactions = transactions.filter((t) => {
    ...
    return (
      amount >= StatsComponent.explorativeAmountMin && amount <= StatsComponent.explorativeAmountMax
    );
  });
  ...
  if (StatsComponent.explorativeDrilldownAccount) {
    transactions = transactions.filter(
      (t) => t.account === StatsComponent.explorativeDrilldownAccount,
    );
  }

  if (StatsComponent.explorativeDrilldownCategory) {
    transactions = transactions.filter(
      (t) => t.category === StatsComponent.explorativeDrilldownCategory,
    );
  }
...
export function checkExplorativeSearchTerm(transaction: any, searchTerm: string): boolean {
  if (
    StatsComponent.explorativeSearchFields.account &&
  ...
  if (StatsComponent.explorativeSearchFields.date) {
    ...
    if (comparisonMatch) {
      ...
      if (dateMatch) {
        ...
        switch (operator) {
          case '>':
            if (transaction.date > isoDate) return true;
            break;
          case '<':
            if (transaction.date < isoDate) return true;
            break;
  ...
  if (StatsComponent.explorativeSearchFields.time) {
    ...
    if (/^\d{1,2}:\d{2}$/.test(timeToCheck)) {
      ...
      if (comparisonMatch) {
        ...
        switch (operator) {
          case '>':
            if (normalizedTransaction > normalizedSearch) return true;
            break;
          case '<':
            if (normalizedTransaction < normalizedSearch) return true;
            break;
  ...
  if (
    StatsComponent.explorativeSearchFields.comment &&
...
export function createExplorativeOverview(container: any, transactions: any[]) {
  container
    ...
    .style('margin-bottom', '15px')
    .text(
      `${StatsComponent.translateService.instant('BI.labelOverview')} – ${StatsComponent.translateService.instant('BI.labelExpenses')} ${StatsComponent.translateService.instant('BI.labelBy')} ${StatsComponent.translateService.instant('BI.labelAccounts')}`,
    );
  ...
  g.selectAll('.bar')
    ...
    .on('click', (event, d) => {
      ...
      StatsComponent.explorativeBreadcrumbs.push({
        level: StatsComponent.translateService.instant('BI.breadcrumbAccount'),
        value: d.account,
      });
      updateBreadcrumbNavigation();
      updateExplorativeView();
...
export function createExplorativeAccountDetail(container: any, transactions: any[]) {
  ...
  // Create table view
  const tableDiv = container.append('div').style('max-height', '400px').style('overflow-y', 'auto');
  ...
  thead
    ...
    .selectAll('th')
    .data([
      StatsComponent.translateService.instant('BI.columnRank'),
      StatsComponent.translateService.instant('BI.columnCategory'),
      StatsComponent.translateService.instant('BI.columnAmount'),
      StatsComponent.translateService.instant('BI.columnShare'),
      StatsComponent.translateService.instant('BI.columnAction'),
    ])
    .enter()
    .append('th')
    .style('padding', '12px')
  ...
  data.forEach((d, i) => {
    const row = tbody.append('tr').style('border-bottom', '1px solid var(--color-muted-light)');
    ...
    row.append('td').style('padding', '10px').style('font-weight', 'bold').text(d.category);
    ...
    row
      ...
      .on('click', (event) => {
        ...
        StatsComponent.explorativeBreadcrumbs.push({
          level: StatsComponent.translateService.instant('BI.breadcrumbCategory'),
          value: d.category,
        });
        updateBreadcrumbNavigation();
        updateExplorativeView();
...
export function createExplorativeCategoryDetail(container: any, transactions: any[]) {
  ...
  container
    ...
    .style('margin-bottom', '15px')
    .text(
      `${StatsComponent.translateService.instant('BI.foundTransactions')} ${data.length} ${StatsComponent.translateService.instant('BI.labelTransactions')} | ${StatsComponent.translateService.instant('BI.labelTotal')}: ${data.reduce((sum, t) => sum + Math.abs(Number(t.amount)), 0).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}`,
    );
...
export function createPeriodComparisonView(container: any, transactions: any[]) {
  ...
  const tableDiv = container.append('div').style('overflow-x', 'auto');
  ...
  const columns = [
    {
      key: 'period',
      label: StatsComponent.translateService.instant('BI.periodComparisonColPeriod'),
    },
    {
      key: 'prevAmount',
      label: StatsComponent.translateService.instant('BI.periodComparisonColPrevMonth'),
    },
    {
      key: 'currAmount',
      label: StatsComponent.translateService.instant('BI.periodComparisonColCurrent'),
    },
    {
      key: 'diff',
      label: StatsComponent.translateService.instant('BI.periodComparisonColDiffAbs'),
    },
    {
      key: 'percentChange',
      label: StatsComponent.translateService.instant('BI.periodComparisonColDiffPct'),
    },
  ...
  comparisons.forEach((comp) => {
    ...
    row
      ...
      .text(
        comp.prevAmount.toLocaleString('de-DE', {
    ...
    row
      ...
      .style('white-space', 'nowrap')
      .text(
        comp.currAmount.toLocaleString('de-DE', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }) +
          ' ' +
    ...
    row
      ...
      .text(
        (comp.diff > 0 ? '+' : '') +
          comp.diff.toLocaleString('de-DE', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }) +
          ' ' +
...
export function createExplorativeTransactionTable(container: any) {
  ...
  const { outliers: allOutliers, recurringTransactions: allRecurring } =
    StatsComponent.detectOutliers(allGlobalTransactions);
  ...
  const allTransactions = getExplorativeFilteredTransactions().filter((t) => Number(t.amount) < 0);
  ...
  displayTransactions.forEach((t) => {
    const row = tbody
      ...
      .on('mouseout', function () {
        d3.select(this).style(
          'background',
          t.isOutlier ? 'var(--color-warning-bg)' : 'var(--color-surface)',
        );
      })
      .on('click', () => {
        if (t.isOutlier) {
    ...
    row.append('td').style('padding', '10px').text(t.account);

    const categoryCell = row.append('td').style('padding', '10px');
    ...
    if (t.isOutlier) {
      ...
      categoryCell.append('span').text(t.category);
    } else {

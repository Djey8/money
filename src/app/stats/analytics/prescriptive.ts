...
export function createScenarioSelector(container: any) {
  ...
  const scenariosDiv = selectorPanel
    .append('div')
    .style('display', 'grid')
  ...
  const scenarios = [
    {
      id: 'optimize-savings-rate',
      label: StatsComponent.translateService.instant('BI.praeskriptiveScenario1'),
      icon: '',
      desc: StatsComponent.translateService.instant('BI.praeskriptiveScenario1Desc'),
    },
    {
      id: 'accelerate-mojo',
      label: StatsComponent.translateService.instant('BI.praeskriptiveScenario2'),
      icon: '',
      desc: StatsComponent.translateService.instant('BI.praeskriptiveScenario2Desc'),
    },
    {
      id: 'savings-goal-simulator',
      label: StatsComponent.translateService.instant('BI.praeskriptiveScenario3'),
      icon: '',
      desc: StatsComponent.translateService.instant('BI.praeskriptiveScenario3Desc'),
    },
  ];
  ...
  scenarios.forEach((scenario) => {
    const card = scenariosDiv
      .append('div')
      .attr('data-scenario-id', scenario.id)
      .style('padding', '20px')
      .style(
        'border',
        '2px solid ' + (StatsComponent.selectedScenario === scenario.id ? '#1976d2' : '#ddd'),
      )
      .style('border-radius', '8px')
      .style(
        'background',
      ...
      .on('mouseover', function () {
        if (StatsComponent.selectedScenario !== scenario.id) {
          d3.select(this).style('border-color', '#1976d2').style('background', '#f5f5f5');
        }
      })
      .on('mouseout', function () {
        if (StatsComponent.selectedScenario !== scenario.id) {
          d3.select(this).style('border-color', '#ddd').style('background', 'var(--color-surface)');
        }
      });
...
export function createOptimizeSavingsRateConfig(container: any) {
  ...
  const subScenarioDiv = container.append('div').style('margin-bottom', '20px');
  ...
  const methodButtons = subScenarioDiv
    .append('div')
    .style('display', 'flex')
    .style('gap', '10px')
    .style('margin-bottom', '20px');
  ...
  const methods = [
    {
      id: 'reduce-category',
      label: StatsComponent.translateService.instant('BI.praeskriptiveMethodReduceCategory'),
    },
    {
      id: 'increase-income',
      label: StatsComponent.translateService.instant('BI.praeskriptiveMethodIncreaseIncome'),
    },
  ];
  ...
  methods.forEach((method) => {
    methodButtons
      ...
      .style(
        'border',
        '2px solid ' +
          (StatsComponent.savingsOptimizationMethod === method.id ? '#1976d2' : '#ddd'),
      )
      ...
      .style(
        'font-weight',
        StatsComponent.savingsOptimizationMethod === method.id ? 'bold' : 'normal',
      )
      .style(
        'color',
        StatsComponent.savingsOptimizationMethod === method.id
...
export function createCategoryReductionControls(container: any) {
  ...
  if (
    StatsComponent.categoryReductions.length === 0 ||
    !StatsComponent.categoryReductions[0].category
  ) {
 ...
 */
export function createSingleCategoryReduction(
  container: any,
  reduction: any,
  index: number,
  ...
  const selectedCategories = new Set(
    StatsComponent.categoryReductions.map((r, i) => (i === index ? '' : r.category)),
  );
  const availableCategories = allCategories.filter((cat) => !selectedCategories.has(cat));

  const select = categoryControl
    .append('select')
    .style('width', '100%')
    .style('padding', '8px')
    .style('border', '1px solid #ddd')
  ...
  categoryControl
    ...
    .style('color', 'var(--color-text-secondary)')
    .html(
      `<strong>Baseline:</strong> Ø ${categoryAverage.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}/Monat`,
    );
  ...
  const methodButtons = methodControl
    .append('div')
    .style('display', 'flex')
    .style('gap', '10px')
    .style('margin-bottom', '20px');
  ...
  methodButtons
    ...
    .style(
      'background',
      reduction.method === 'percentage' ? 'var(--color-info-surface)' : 'var(--color-surface)',
    )
    .style('border-radius', '6px')
    .style('cursor', 'pointer')
    .style('font-size', '14px')
  ...
  methodButtons
    ...
    .style(
      'background',
      reduction.method === 'fixed' ? 'var(--color-info-surface)' : 'var(--color-surface)',
    )
    .style('border-radius', '6px')
    .style('cursor', 'pointer')
    .style('font-size', '14px')
  ...
  if (reduction.method === 'percentage') {
    ...
    inputGroup.append('span').style('color', 'var(--color-text-secondary)').text('%');
...
export function createIncomeIncreaseControls(container: any) {
  ...
  const lastMonthIncomes = AppStateService.instance.allTransactions.filter(
    (t) => t.date.startsWith(lastMonthStr) && Number(t.amount) > 0,
  );
 ...
 */
export function createSingleIncomeIncrease(
  container: any,
  increase: any,
  index: number,
  ...
  methodButtons
    ...
    .style(
      'background',
      increase.method === 'fixed' ? 'var(--color-info-surface)' : 'var(--color-surface)',
    )
    .style('border-radius', '6px')
    .style('cursor', 'pointer')
    .style('font-size', '14px')
  ...
  methodButtons
    ...
    .style(
      'background',
      increase.method === 'percentage' ? 'var(--color-info-surface)' : 'var(--color-surface)',
    )
    .style('border-radius', '6px')
    .style('cursor', 'pointer')
    .style('font-size', '14px')
  ...
  if (increase.method === 'fixed') {
    ...
    if (currentSalary > 0) {
      ...
      entryDiv
        ...
        .style('color', 'var(--color-text-secondary)')
        .html(
          `<strong>Neues Gehalt:</strong> ${newSalary.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency} (+${(newSalary - currentSalary).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency})`,
        );
    }
  }
}
...
export function createAccelerateMojoConfig(container: any) {
  ...
  const mojoTransactions = AppStateService.instance.allTransactions.filter(
    (t) =>
      t.category === '@Mojo' &&
      Number(t.amount) < 0 &&
  ...
  mojoTransactions.forEach((t) => {
    ...
    monthlyContributions.set(
      month,
      (monthlyContributions.get(month) || 0) + Math.abs(Number(t.amount)),
    );
  });

  const avgMonthlyContribution =
    monthlyContributions.size > 0
      ? Array.from(monthlyContributions.values()).reduce((sum, v) => sum + v, 0) /
        monthlyContributions.size
      : 0;
  ...
  const baselineMonthsToGoal =
    avgMonthlyContribution > 0 ? Math.ceil(remainingAmount / avgMonthlyContribution) : Infinity;
  ...
  // Method selector
  const controlsDiv = container.append('div').style('max-width', '500px');
  ...
  const methodButtons = controlsDiv
    .append('div')
    .style('display', 'flex')
    .style('gap', '10px')
    .style('margin-bottom', '20px');
  ...
  methodButtons
    ...
    .style(
      'border',
      '2px solid ' + (StatsComponent.mojoIncreaseMethod === 'fixed' ? '#1976d2' : '#ddd'),
    )
    .style(
      'background',
      StatsComponent.mojoIncreaseMethod === 'fixed'
  ...
  methodButtons
    ...
    .style(
      'border',
      '2px solid ' + (StatsComponent.mojoIncreaseMethod === 'percentage' ? '#1976d2' : '#ddd'),
    )
    .style(
      'background',
...
export function createSavingsGoalSimulatorConfig(container: any) {
  ...
  const goalInputGroup = goalControl
    .append('div')
    .style('display', 'flex')
    .style('gap', '10px')
    .style('align-items', 'center');
  ...
  const optimizationDiv = container.append('div').style('margin-top', '20px');
...
export function createOptimizeSavingsRateResults(container: any) {
  ...
  const monthlyDataForRate = new Map<
    string,
    { income: number; expenses: number; categoryExpenses: Map<string, number> }
  >();
  ...
  if (StatsComponent.savingsOptimizationMethod === 'reduce-category') {
    ...
    StatsComponent.categoryReductions.forEach((reduction) => {
      ...
      const categoryAmount =
        categoryMonthlyTotals.reduce((sum, v) => sum + v, 0) / numMonthsForCalc;
      ...
      if (reduction.method === 'percentage') {
        reductionDescriptions.push(
          `${reduction.category} -${reduction.percent}% (-${reductionAmount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency})`,
        );
      } else {
        const reductionPercent = categoryAmount > 0 ? (reductionAmount / categoryAmount) * 100 : 0;
        reductionDescriptions.push(
          `${reduction.category} -${reductionAmount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency} (-${reductionPercent.toFixed(1)}%)`,
        );
      }
    });
    ...
    StatsComponent.incomeIncreases.forEach((increase, index) => {
      ...
      if (increase.method === 'percentage') {
        ...
        increaseDescriptions.push(
          `Einkommen +${increase.percent}% (+${increaseAmount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}, neues Gehalt: ${newSalary.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency})`,
        );
      } else {
        increaseDescriptions.push(
          `Einkommen +${increaseAmount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}`,
        );
      }
    });
  ...
  const scenarioSavingsRate =
    scenarioTotalIncome > 0
  ...
  const comparisonDiv = resultsPanel
    .append('div')
    .style('display', 'grid')
  ...
  createComparisonBox(
    comparisonDiv,
    StatsComponent.translateService.instant('BI.savingsRate'),
  ...
  createComparisonBox(
    comparisonDiv,
    StatsComponent.translateService.instant('BI.savingsGoalLabel'),
    baselineMonthlySavings.toLocaleString('de-DE', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    }) +
      ' ' +
      AppStateService.instance.currency,
    scenarioMonthlySavings.toLocaleString('de-DE', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    }) +
      ' ' +
...
export function createAccelerateMojoResults(container: any) {
  ...
  const baselineMonthsToGoal =
    baselineContribution > 0 ? Math.ceil(remainingAmount / baselineContribution) : Infinity;
  ...
  const scenarioMonthsToGoal =
    scenarioContribution > 0 ? Math.ceil(remainingAmount / scenarioContribution) : Infinity;
  const timeSaved =
    baselineMonthsToGoal !== Infinity && scenarioMonthsToGoal !== Infinity
  ...
  createComparisonBox(
    comparisonDiv,
    StatsComponent.translateService.instant('BI.monthlyDeposit'),
    baselineContribution.toLocaleString('de-DE', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    }) +
      ' ' +
      AppStateService.instance.currency,
    scenarioContribution.toLocaleString('de-DE', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    }) +
      ' ' +
  ...
  if (baselineMonthsToGoal !== Infinity) {
    createComparisonBox(
      comparisonDiv,
      StatsComponent.translateService.instant('BI.monthsToGoal'),
  ...
  if (timeSaved > 0) {
    const highlightBox = resultsPanel
      .append('div')
      .style('padding', '20px')
...
export function createSavingsGoalSimulatorResults(container: any) {
  ...
  const baselineMonthsToGoal =
    baselineMonthlySavings > 0 ? Math.ceil(goalAmount / baselineMonthlySavings) : Infinity;
  const scenarioMonthsToGoal =
    scenarioMonthlySavings > 0 ? Math.ceil(goalAmount / scenarioMonthlySavings) : Infinity;
  const timeSaved =
    baselineMonthsToGoal !== Infinity && scenarioMonthsToGoal !== Infinity
      ? baselineMonthsToGoal - scenarioMonthsToGoal
      : 0;
  ...
  if (StatsComponent.savingsOptimizationMethod === 'reduce-category') {
    ...
    StatsComponent.categoryReductions.forEach((reduction) => {
      if (reduction.method === 'percentage') {
        ...
        reductionDescriptions.push(
          `${reduction.category} -${reduction.amount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}`,
        );
      }
    });
    ...
    StatsComponent.incomeIncreases.forEach((increase) => {
      if (increase.method === 'percentage') {
        ...
        increaseDescriptions.push(
          `Einkommen +${increase.amount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}`,
        );
      }
    });
  ...
  if (timeSaved > 0) {
    ...
    highlightBox
      ...
      .style('color', 'var(--color-success-dark)')
      .text(
        `${timeSaved} ${timeSaved === 1 ? StatsComponent.translateService.instant('BI.months').slice(0, -1) : StatsComponent.translateService.instant('BI.months')} ${StatsComponent.translateService.instant('BI.faster')}`,
      );
  }
...
export function createTimelineChart(
  ...
  const chartDiv = container.append('div').style('margin-top', '20px');
  ...
  svg
    ...
    .style('fill', '#1976d2')
    .text(
      `${StatsComponent.translateService.instant('BI.current')}: ${baselineMonths} ${StatsComponent.translateService.instant('BI.months')}`,
    );
  ...
  svg
    ...
    .style('fill', '#4caf50')
    .text(
      `${StatsComponent.translateService.instant('BI.scenario')}: ${scenarioMonths} ${StatsComponent.translateService.instant('BI.months')}`,
    );
...
export function createComparisonChart(
  ...
  const chartDiv = container.append('div').style('margin-top', '20px').style('width', '100%');
  ...
  const chartsContainer = chartDiv
    .append('div')
    .style('display', 'grid')
 ...
 */
export function createSingleMetricChart(
  container: any,
  title: string,
  baseline: number,
  scenario: number,
  ...
  setTimeout(() => {
    ...
    const data = [
      {
        label: StatsComponent.translateService.instant('BI.current'),
        value: baseline,
        color: '#1976d2',
      },
      {
        label: StatsComponent.translateService.instant('BI.scenario'),
        value: scenario,
        color: '#4caf50',
      },
    ];
...
export function refreshScenario() {
  ...
  d3.select('#praeskriptive-main-container')
    .selectAll('[data-scenario-id]')
    .each(function () {

...
export function createMojoGoalForecast(container: any) {
  ...
  const mojoTransactions = AppStateService.instance.allTransactions.filter(
    (t) => t.category === '@Mojo' && Number(t.amount) < 0 && new Date(t.date) >= threeMonthsAgo,
  );
  ...
  balanceBox
    ...
    .style('color', '#1976d2')
    .text(
      currentMojoBalance.toLocaleString('de-DE', {
  ...
  contribBox
    ...
    .style('color', '#7b1fa2')
    .text(
      avgMonthlyContribution.toLocaleString('de-DE', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }) +
        ' ' +
  ...
  goalBox
    ...
    .style('color', '#f57c00')
    .text(
      mojoGoalAmount.toLocaleString('de-DE', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }) +
        ' ' +
  ...
  // Prediction
  const predictionBox = resultsDiv
    .append('div')
    .style('padding', '15px')
    .style(
      'background',
      monthsToGoal !== Infinity ? 'var(--color-success-surface-light)' : '#ffebee',
    )
    .style('border-radius', '8px')
    .style('text-align', 'center');
  ...
  if (currentMojoBalance >= mojoGoalAmount) {
    ...
    predictionBox
      ...
      .text(
        targetDate
          ? `${targetDate.toLocaleDateString('de-DE', { month: 'long', year: 'numeric' })}`
          : 'N/A',
      );
...
export function createMetricPredictionSelector(container: any) {
  ...
  const metricsDiv = selectorPanel
    .append('div')
    .style('display', 'flex')
    .style('gap', '10px')
  ...
  const metrics = [
    ...
    { id: 'net-worth', label: StatsComponent.translateService.instant('BI.netWorth') },
    {
      ...
      label: StatsComponent.translateService.instant('BI.categorySpending'),
    },
  ];

  metrics.forEach((metric) => {
    const button = metricsDiv
      .append('button')
      ...
      .style(
        'border',
        '2px solid ' + (StatsComponent.predictiveSelectedMetric === metric.id ? '#1976d2' : '#ddd'),
      )
      .style(
        'background',
      ...
      .style(
        'font-weight',
        StatsComponent.predictiveSelectedMetric === metric.id ? 'bold' : 'normal',
      )
      .text(metric.label)
      .on('click', () => {
        StatsComponent.predictiveSelectedMetric = metric.id;
  ...
  if (StatsComponent.predictiveSelectedMetric === 'category-spending') {
    const categorySelectorDiv = selectorPanel
      .append('div')
    ...
    AppStateService.instance.allTransactions
      ...
      .forEach((t) => {
        categoryTotals.set(
          t.category,
          (categoryTotals.get(t.category) || 0) + Math.abs(Number(t.amount)),
        );
      });
    ...
    if (
      (!StatsComponent.predictiveSelectedCategory ||
        StatsComponent.predictiveSelectedCategory === '') &&
      categories.length > 0
    ) {
    ...
    const select = categorySelectorDiv
      .append('select')
      .style('width', '100%')
      .style('padding', '8px')
...
export function createPredictionChart(container: any) {
  ...
  */
  ...
  const exponentialWithConnection = [
...
export function createPredictionSummaryTable(container: any) {
  ...
  const tableDiv = tablePanel.append('div').style('overflow-x', 'auto');
  ...
  predictions.linear.forEach((linearPred: any, i: number) => {
    ...
    row.append('td').style('padding', '10px').style('font-weight', 'bold').text(linearPred.month);

    row.append('td').style('padding', '10px').text(linearPred.value.toFixed(2));

    row.append('td').style('padding', '10px').text(expPred.value.toFixed(2));

    const arimaCell = row.append('td').style('padding', '10px').style('font-weight', 'bold');

    if (arimaPred) {
      arimaCell.style('color', '#00bcd4').text(arimaPred.value.toFixed(2));
    } else {
      arimaCell
        .style('color', '#ff9800')
    ...
    if (i === 0) {
      ...
      const change =
        ((ensemblePred.value - lastHistorical) / Math.max(Math.abs(lastHistorical), 0.01)) * 100;
    ...
    row.append('td').style('padding', '10px').style('font-size', '20px').text(trend);
  });
}
/**
...
export function getPredictiveHistoricalData(metric: string): any[] {
  ...
  const startDate = new Date(
    today.getFullYear(),
    today.getMonth() - StatsComponent.predictiveMonthsHistory,
    1,
  );
  ...
  switch (metric) {
    case 'savings-rate': {
      ...
      transactions
        .filter((t) => new Date(t.date) >= startDate && t.date.substring(0, 7) !== currentMonthStr)
        .forEach((t) => {
          const month = t.date.substring(0, 7);
    ...
    case 'category-spending': {
      ...
      if (selectedCategory) {
        ...
        categoryTransactions.forEach((t) => {
          ...
          monthlySpending.set(
            month,
            (monthlySpending.get(month) || 0) + Math.abs(Number(t.amount)),
          );
        });
...
export function calculatePredictions(historicalData: any[]): any {
  ...
  for (let i = 1; i <= StatsComponent.predictiveMonthsFuture; i++) {
    ...
    const predictionError =
      standardError * Math.sqrt(1 + 1 / n + distanceFromMean / (n * varX + 1e-10));
  ...
  for (let i = 0; i < linearPredictions.length; i++) {
    ...
    if (arimaPredictions.length > i) {
      ...
      ensembleValue =
        (linearPredictions[i].value + exponentialPredictions[i].value + arimaPredictions[i].value) /
        3;
    } else {

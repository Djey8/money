...
export function createKPI(kpiType: string) {
  switch (kpiType) {
    ...
    case 'top-spending':
      return createTopSpendingCategories(
        StatsComponent.period,
        StatsComponent.Index,
        StatsComponent.numCategories,
      );
    case 'savings-rate':
...
export function createNetWorthTrendChart() {
  ...
  const rawData = Array.from(grouped.entries()).sort(
  ...
  svg
    ...
    .on('click', function (event, d) {
      ...
      const transactions = d.transactions.sort(
        (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
      );
      const detailsHtml = transactions
        .map(
          (t) => `
...
export function createExpenseIncomeRatio() {
  ...
  const transactions = ChartFilterService.filterTransactions(StatsComponent.chartFilter).map(
    (t) => ({
    ...
    .rollups(
      ...
      (values) => {
        ...
        const expenses = Math.abs(
          values.filter((t) => t.account !== 'Income').reduce((sum, t) => sum + t.amount, 0),
        );
  ...
  svg
    ...
    .on('mouseenter', function (event, d) {
      ...
      if (d3.select('#kpi-info-box').empty()) {
        ...
        const side = dotX < window.innerWidth / 2 ? 'right' : 'left';
        const left = side === 'right' ? dotX + 12 : dotX - 200; // box width
    ...
    .on('click', function (event, d) {
      ...
      const transactions = AppStateService.instance.allTransactions
        .filter((t) => {
          ...
          return (
            `${tDate.getFullYear()}-${(tDate.getMonth() + 1).toString().padStart(2, '0')}` ===
            monthStr
          );
        })
...
export function createIncomeStreamsBreakdown(selectedPeriod = 'all', selectedIndex = 0) {
  ...
  function createChart(filter = null) {
    ...
    for (const acc of allocations) {
      ...
      const spent = Array.from(categoryMap.values()).reduce(
    ...
    if (filter === 'income') {
      ...
      filteredLinks = links.filter(
        (l) => accountIndices.has(l.source) && accountIndices.has(l.target),
      );
      ...
      const expenseNodes = nodes.filter(
        (n) => n.type === 'account' || n.type === 'expense' || n.type === 'cashflow',
      );
      ...
      filteredLinks = links.filter((l) => {
        ...
        if (
          sourceNode.type === 'account' &&
          (targetNode.type === 'expense' || targetNode.type === 'cashflow')
        ) {
      ...
      filteredLinks.forEach((l) => {
        ...
        nodeTotalValues.set(targetName, (nodeTotalValues.get(targetName) || 0) + l.value);
      });
    ...
    if (filter === 'expense' || !filter) {
      ...
      filteredNodes.forEach((node) => {
        if (node.type === 'expense' || node.type === 'cashflow') {
          const incomingLinks = remappedLinks.filter(
            (l) => filteredNodes[l.target].name === node.name,
          );
      ...
      .text((d) => {
        ...
        if (originalNodeIndex >= 0) {
          if (nodeType === 'income') {
            ...
            value = d3.sum(
              fullData.links.filter((l) => fullData.nodes[l.source].name === nodeName),
              (l) => l.value,
            );
          } else if (nodeType === 'account') {
            // For account nodes, sum all incoming links
            value = d3.sum(
              fullData.links.filter((l) => fullData.nodes[l.target].name === nodeName),
              (l) => l.value,
            );
          } else {
  ...
  svg
    ...
    .on('click', function (event, d) {
      ...
      const transactions = d.transactions
        .filter(
          (t: any) =>
            `${t.date.getFullYear()}-${(t.date.getMonth() + 1).toString().padStart(2, '0')}` ===
            monthStr,
        )
 ...
 */
export function createTopSpendingCategories(
  selectedPeriod = 'all',
  selectedIndex = 0,
  numCategories = 5,
) {
  ...
  svg
    ...
    .on('click', function (event, d) {
      ...
      d3.select('body').on(
        ...
        function (e) {
          if (
            !d3.select(e.target).classed('expand-dot') &&
            !d3.select(e.target).attr('id')?.startsWith('expand-details')
          ) {
            d3.select('#kpi-info-box').remove();
            d3.select('body').on('click.kpi-info', null);
          }
  ...
  const categorySelectContainer = d3
    .select('#chart-container')
    .append('div')
    .style('position', 'absolute')
    .style('top', '70px')
  ...
  const categorySelect = categorySelectContainer
    .append('select')
...
export function createRecurringVsOneTimeChart(selectedPeriod = 'all', selectedIndex = 0) {
  ...
  filteredTransactions
    ...
    .forEach((t) => {
      ...
      const isRecurring = AppStateService.instance.allSubscriptions.some(
        (sub) =>
          sub.title === title &&
  ...
  pieGroups
    ...
    .on('click', function (event, d) {
      ...
      html +=
        "<div style='max-height:180px;overflow:auto;'><ul style='margin:0;padding-left:18px;'>";
      d.data.categories
...
export function createHeatmapCalendar(
  ...
  const horizontalOffset =
    viewMode === 'month' ? (width - weekCount * (cellSize + cellMargin)) / 2 : 0;
  ...
  const heatmapCells = svg
    ...
    .on('mouseover', function (event, d) {
      ...
      d3.select(this).attr('stroke', 'var(--color-text)').attr('stroke-width', 2);
      ...
      tooltip.style('left', `${left}px`).style('top', `${event.pageY - 30}px`);
  ...
  const shouldRepositionLegends =
    height + legendRequiredHeight > availableHeight || height < minHeatmapHeight;
  ...
  const totalDaysWithTransactions = Array.from(dailySumsMap.values()).filter(
    (v) => Number(v) !== 0,
  ).length;
  const totalIncome = Number(
    values.filter((v) => Number(v) > 0).reduce((sum, v) => Number(sum) + Number(v), 0),
  );
  const totalExpenses = Number(
    values.filter((v) => Number(v) < 0).reduce((sum, v) => Number(sum) + Math.abs(Number(v)), 0),
  );
  ...
  const dailyNets = dateRange.map(
    (d) => Number(dailySumsMap.get(d3.timeFormat('%Y-%m-%d')(d))) || 0,
  );
  const variance =
    dailyNets.reduce((sum, val) => sum + Math.pow(val - avgNetPerDay, 2), 0) / dailyNets.length;
  const standardDeviation = Math.sqrt(variance);
  const coefficientOfVariation =
    avgNetPerDay !== 0 ? (standardDeviation / Math.abs(avgNetPerDay)) * 100 : 0;
  ...
  const dayOfWeekAvgs = dayOfWeekSums.map((sum, i) =>
    dayOfWeekCounts[i] > 0 ? sum / dayOfWeekCounts[i] : 0,
  );
  const bestDayOfWeek = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][
    dayOfWeekAvgs.indexOf(Math.max(...dayOfWeekAvgs))
  ];
  const worstDayOfWeek = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][
    dayOfWeekAvgs.indexOf(Math.min(...dayOfWeekAvgs))
  ];
  ...
  const secondHalfAvg =
    dailyNets.slice(midPoint).reduce((sum, v) => sum + v, 0) / (dailyNets.length - midPoint);
  ...
  let longestTransactionStreak = 0,
  ...
  const maxExpenseDay = Math.abs(
    d3.min(filteredTransactions, (d) => (d.amount < 0 ? d.amount : 0)) || 0,
  );

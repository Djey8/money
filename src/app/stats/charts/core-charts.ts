...
export function createZoomableChart() {
  ...
  const filteredTransactions = ChartFilterService.filterTransactions(
    StatsComponent.chartFilter,
    false,
  );
  ...
  const mapToArray = (m: Map<string, number>) =>
    Array.from(m.entries()).map(([tag, amount]) => ({ tag, amount }));
  ...
  if (expensesChildren.length > 0) {
    data.children.push({
      name: `${StatsComponent.translatedIncomeValues[9]} ${expensesChildren
        .reduce(
          (sum, child) =>
            sum +
            child.children
  ...
  const svg = d3
    ...
    .attr(
      'style',
      `max-width: 100%; height: auto; margin: auto; display: block; background: transperant; cursor: pointer;`,
    )
  ...
  const node = svg
    ...
    .on('mouseover', function () {
      d3.select(this).attr('stroke', 'var(--color-text)').attr('stroke-width', 1.5);
    })
...
export function createHistogramChart() {
  ...
  const generateBins = (domain: [number, number], binCount: number) => {
    ...
    const bins = d3.histogram().domain(domain).thresholds(binThresholds)(data);
    ...
    const extraRightBin = {
      ...
      x1: bins[bins.length - 1].x1 + (bins[0].x1 - bins[0].x0),
      length: 0,
    };
  ...
  const zoom = d3
    ...
    .translateExtent([
      [0, 0],
      [width, height],
    ]) // Restrict panning to chart area
    .on('zoom', (event) => {
      ...
      bars
        ...
        .attr('width', (d: d3.Bin<number, number>) =>
          Math.max(1, newX(d.x1 || 0) - newX(d.x0 || 0) - 1),
        )
        .attr('height', (d: d3.Bin<number, number>) => height - y(d.length))
...
export function createCashflowBarChart(period?, selectedYear?) {
  ...
  const groupByPeriod = (
    ...
    const grouped: Map<Date | number, { date: string; amount: number }[]> = d3.group(
      transactions,
      (transaction) => {
        const date = new Date(transaction.date);
        if (period === 'week')
          return `W${(parseInt(d3.timeFormat('%U')(date)) + 1).toString().padStart(2, '0')} ${date.getFullYear()}`;
  ...
  if (groupPeriod === 'week') {
    data.sort((a, b) => {
      const [weekA, yearA] = a.period
        .toString()
        .split(' ')
        .map((part) => part.trim());
      const [weekB, yearB] = b.period
        .toString()
        .split(' ')
        .map((part) => part.trim());
      return (
        new Date(`${yearA}-01-01`).getTime() +
        parseInt(weekA.slice(1)) * 7 * 24 * 60 * 60 * 1000 -
        (new Date(`${yearB}-01-01`).getTime() + parseInt(weekB.slice(1)) * 7 * 24 * 60 * 60 * 1000)
      );
    });
  } else if (groupPeriod === 'quarter') {
    data.sort((a, b) => {
      const [quarterA, yearA] = a.period
        .toString()
        .split(' ')
        .map((part) => part.trim());
      const [quarterB, yearB] = b.period
        .toString()
        .split(' ')
        .map((part) => part.trim());
      return (
        new Date(`${yearA}-01-01`).getTime() +
        parseInt(quarterA.slice(1)) * 3 * 30 * 24 * 60 * 60 * 1000 -
        (new Date(`${yearB}-01-01`).getTime() +
          parseInt(quarterB.slice(1)) * 3 * 30 * 24 * 60 * 60 * 1000)
      );
    });
  } else if (groupPeriod === 'month') {
    data.sort((a, b) => {
      const [monthA, yearA] = a.period
        .toString()
        .split(' ')
        .map((part) => part.trim());
      const [monthB, yearB] = b.period
        .toString()
        .split(' ')
        .map((part) => part.trim());
      const monthMap = {
...
export function createCategoryBubbleChart(selectedPeriod = 'all', selectedIndex = 0) {
  ...
  const color = d3
    ...
    .range(
      data.map((d) => {
        const account = AppStateService.instance.allTransactions.find(
          (t) => t.category.replace(/@/g, '') === d.id,
        )?.account;
  ...
  const root = pack(
    d3.hierarchy({ children: data }).sum((d: { value: number }) => Math.abs(d.value)),
  ...
  // Add a label
  const text = node.append('text').attr('clip-path', (d: { r: number }) => `circle(${d.r})`);
...
export function createChart(
  ...
  for (const t of transactions) {
    ...
    if (isChecked && t.amount < 0 && (t.original.category || '') === '@Mojo') {
    ...
    if (filter === 'income') {
      ...
      value =
        t.account === 'Income' ? (t.amount * AppStateService.instance.splurge) / 100 : t.amount;
  ...
  if (chartState.filterType !== 'all') {
    ...
    if (chartState.filterType === 'custom' && chartState.customDateStart) {
      ...
      const range = ChartFilterService.getDateRange(
        chartState.filterType,
        chartState.selectedIndex,
      );
      ...
      cumulative = allFiltered
        ...
        .reduce((sum, t) => {
          ...
          if (filter === 'income') value = amount;
            ...
            value =
              t.account === 'Income' ? (amount * AppStateService.instance.daily) / 100 : amount;
          else if (filter === 'splurge' && (t.account === 'Income' || t.account === 'Splurge'))
            value =
              t.account === 'Income' ? (amount * AppStateService.instance.splurge) / 100 : amount;
          else if (filter === 'smile' && (t.account === 'Income' || t.account === 'Smile'))
            value =
              t.account === 'Income' ? (amount * AppStateService.instance.smile) / 100 : amount;
          else if (filter === 'fire' && (t.account === 'Income' || t.account === 'Fire'))
            value =
              t.account === 'Income' ? (amount * AppStateService.instance.fire) / 100 : amount;
          return sum + value;
        }, 0);
    }
  }
...
export function showTransactionDetails(anyTransactions: any[], dateStr: string, filter: string) {
  ...
  const transactionList = modal.append('ul').style('list-style-type', 'none').style('padding', '0');

  anyTransactions
    .filter((t) => t.account.toLowerCase() === filter || filter === 'income')
    .forEach((transaction) => {
      const listItem = transactionList
        .append('li')
      ...
      if (
        ...
        if (smileProject) {
          ...
          const totalTarget =
            smileProject.buckets?.reduce((sum: number, b: any) => sum + (b.target || 0), 0) || 0;
          const totalAmount =
            smileProject.buckets?.reduce((sum: number, b: any) => sum + (b.amount || 0), 0) || 0;
          listItem
            .append('div')
      ...
      if (
        ...
        if (growProject) {
          ...
          if (growProject.description)
            html += `<span style="color:var(--color-text-secondary);">${growProject.description}</span><br/>`;
          if (growProject.sub) html += `Title: ${growProject.sub}<br/>`;
          if (growProject.status) html += `Status: ${growProject.status}<br/>`;
          if (growProject.strategy !== undefined) html += `Strategy: ${growProject.strategy}<br/>`;
          if (growProject.risks) html += `Risks: ${growProject.risks}<br/>`;
          ...
          listItem.append('div').style('margin-top', '6px').style('color', '#388e3c').html(html);
        }
      }
    });
...
export function createPieChart() {
  ...
  const data = [
    ...
    {
      label: `${Math.round(((splurgeValue * 100) / totalAmount) * 100) / 100} %`,
      value: splurgeValue,
    },
  ...
  const color = d3.scaleOrdinal().range(d3.schemeCategory10); // Use a predefined color scheme from D3.js
  ...
  svg
    ...
    .attr(
      'style',
      'max-width: 100%; height: auto; font: 10px sans-serif; display: block; margin: 0 auto;',
    );

  const pie = d3.pie().value((d: any) => Math.abs(d.value));
  ...
  arcs
    ...
    .attr('fill', (d: any, i: number) =>
      d.data.value >= 0 ? customColors[i] : customColorsMinus[i],
    ); // Use the customColors array for positive values and customColorsMinus array for negative values
  ...
  svg
    ...
    .attr('font-size', '16px')
    .text(function (d, i) {
      return StatsComponent.translatedValues[i];
    })

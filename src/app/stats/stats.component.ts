...
setTimeout(() => import('../app.component').then((m) => (AppComponent = m.AppComponent)));
...
setTimeout(() =>
  import('../main/home/home.component').then((m) => (HomeComponent = m.HomeComponent)),
);
...
import {
  ...
  createExpenseIncomeRatio,
  createIncomeStreamsBreakdown,
  createSavingsRate as createSavingsRateKPI,
  createBurnRate,
  createTopSpendingCategories,
  createRecurringVsOneTimeChart,
  createHeatmapCalendar,
} from './charts/kpi-charts';
import {
  createZoomableChart,
  createHistogramChart,
  createCashflowBarChart,
  createCategoryBubbleChart,
  createChart as createAccountChart,
  showTransactionDetails,
  createPieChart,
} from './charts/core-charts';
import {
  createBIDashboard,
  createAnalyticsLevelSelector,
...
} from './bi/bi-dashboard';
import {
  createExplorativeAnalytics,
  updateExplorativeView,
  updateBreadcrumbNavigation,
  getExplorativeFilteredTransactions,
  checkExplorativeSearchTerm,
} from './analytics/explorative';
import {
  createPraediktiveAnalytics,
  refreshPredictiveMetrics,
  getPredictiveHistoricalData,
  calculatePredictions,
  calculateARIMAPredictions,
...
export class StatsComponent implements AfterViewInit, AfterViewChecked, OnDestroy, OnInit {
  ...
  static explorativeSearchFields = {
  ...
  constructor(
    ...
    d3Service: D3VisualizationService,
  ) {
  ...
  get customDateStart() {
    return StatsComponent.customDateStart;
  }
  set customDateStart(value: string) {
    StatsComponent.customDateStart = value;
  }

  get customDateEnd() {
    return StatsComponent.customDateEnd;
  }
  set customDateEnd(value: string) {
    StatsComponent.customDateEnd = value;
  }

  get explorativeAmountMin() {
    return StatsComponent.explorativeAmountMin;
  }
  set explorativeAmountMin(value: number) {
    StatsComponent.explorativeAmountMin = value;
  }

  get explorativeAmountMax() {
    return StatsComponent.explorativeAmountMax;
  }
  set explorativeAmountMax(value: number) {
    StatsComponent.explorativeAmountMax = value;
  }

  get explorativeSearchText() {
    return StatsComponent.explorativeSearchText;
  }
  set explorativeSearchText(value: string) {
    StatsComponent.explorativeSearchText = value;
  }

  get explorativeSearchInDate() {
    return StatsComponent.explorativeSearchFields.date;
  }
  set explorativeSearchInDate(value: boolean) {
    StatsComponent.explorativeSearchFields.date = value;
  }

  get explorativeSearchInAmount() {
    return StatsComponent.explorativeSearchFields.amount;
  }
  set explorativeSearchInAmount(value: boolean) {
    StatsComponent.explorativeSearchFields.amount = value;
  }

  get explorativeSearchInDescription() {
    return StatsComponent.explorativeSearchFields.comment;
  }
  set explorativeSearchInDescription(value: boolean) {
    StatsComponent.explorativeSearchFields.comment = value;
  }

  get explorativeSearchInAccount() {
    return StatsComponent.explorativeSearchFields.account;
  }
  set explorativeSearchInAccount(value: boolean) {
    StatsComponent.explorativeSearchFields.account = value;
  }

  get explorativeSearchInCategory() {
    return StatsComponent.explorativeSearchFields.category;
  }
  set explorativeSearchInCategory(value: boolean) {
    StatsComponent.explorativeSearchFields.category = value;
  }

  get explorativeSearchHelpVisible() {
    return StatsComponent.explorativeSearchHelpVisible;
  }
  set explorativeSearchHelpVisible(value: boolean) {
    StatsComponent.explorativeSearchHelpVisible = value;
  }

  // Chart filter bindings
  get chartFilterVisible() {
    return StatsComponent.chartFilterVisible;
  }
  get chartFilterAdvanced() {
    return StatsComponent.chartFilterAdvanced;
  }
  get chartSearchHelpVisible() {
    return StatsComponent.chartSearchHelpVisible;
  }
  get chartFilter() {
    return StatsComponent.chartFilter;
  }
  get showChartFilter() {
  ...
  selectChartCustom(): void {
    ...
    if (!StatsComponent.chartFilter.customDateStart) {
      const dates = AppStateService.instance.allTransactions
        ...
        .filter((d) => d)
        .sort();
      StatsComponent.chartFilter.customDateStart =
      ...
      if (range) {
        if (filter.filterType === 'year') {
          ...
          const weekNum = Math.ceil(
            ((range.startDate.getTime() - new Date(range.startDate.getFullYear(), 0, 1).getTime()) /
              86400000 +
              new Date(range.startDate.getFullYear(), 0, 1).getDay() +
              1) /
              7,
          );
  ...
  isFixedCost(category: string): boolean {
    return AppStateService.instance.allSubscriptions.some(
      (sub) => sub.category.replace('@', '') === category,
    );
  }
  ...
  appendToChartSearch(snippet: string): void {
    if (StatsComponent.chartFilter.searchText && StatsComponent.chartFilter.searchText.trim()) {
      StatsComponent.chartFilter.searchText =
        StatsComponent.chartFilter.searchText.trim() + ' && ' + snippet;
    } else {
  ...
  appendToExplorativeSearch(snippet: string): void {
    if (StatsComponent.explorativeSearchText && StatsComponent.explorativeSearchText.trim()) {
      StatsComponent.explorativeSearchText =
        StatsComponent.explorativeSearchText.trim() + ' && ' + snippet;
    } else {
  ...
  showInsights() {
    ...
    StatsComponent.createChart(
      StatsComponent.modus,
      StatsComponent.selectedYear,
      this.isChecked,
      StatsComponent.selectedMode,
  ...
  static createKPI(kpiType: string) {
    return createKPI(kpiType);
  }
  static createNetWorthTrendChart() {
    return createNetWorthTrendChart();
  }
  static createExpenseIncomeRatio() {
    return createExpenseIncomeRatio();
  }
  static createIncomeStreamsBreakdown(p?: string, i?: number) {
    return createIncomeStreamsBreakdown(p, i);
  }
  static createSavingsRate() {
    return createSavingsRateKPI();
  }
  static createBurnRate() {
    return createBurnRate();
  }
  static createTopSpendingCategories(p?: string, i?: number, n?: number) {
    return createTopSpendingCategories(p, i, n);
  }
  static createRecurringVsOneTimeChart(p?: string, i?: number) {
    return createRecurringVsOneTimeChart(p, i);
  }
  static createHeatmapCalendar(y?: number, m?: number | null, v?: string) {
    return createHeatmapCalendar(y, m, v as any);
  }
  static createZoomableChart() {
    return createZoomableChart();
  }
  static createHistogramChart() {
    return createHistogramChart();
  }
  static createCashflowBarChart(p?: any, y?: any) {
    return createCashflowBarChart(p, y);
  }
  static createCategoryBubbleChart(p?: string, i?: number) {
    return createCategoryBubbleChart(p, i);
  }
  static createChart(
    filter: string,
    selectedYear = 'all',
    ...
    return createAccountChart(filter, selectedYear, isChecked, selectedMode, selectedMonth);
  }
  static showTransactionDetails(t: any[], d: string, f: string) {
    return showTransactionDetails(t, d, f);
  }
  static createPieChart() {
    return createPieChart();
  }
  static createBIDashboard(n: number) {
    return createBIDashboard(n);
  }
  static createExplorativeAnalytics(c: any) {
    return createExplorativeAnalytics(c);
  }
  static updateExplorativeView() {
    return updateExplorativeView();
  }
  static updateBreadcrumbNavigation() {
    return updateBreadcrumbNavigation();
  }
  static getExplorativeFilteredTransactions() {
    return getExplorativeFilteredTransactions();
  }
  static checkExplorativeSearchTerm(t: any, s: string) {
    return checkExplorativeSearchTerm(t, s);
  }
  static createPraediktiveAnalytics(c: any) {
    return createPraediktiveAnalytics(c);
  }
  static refreshPredictiveMetrics() {
    return refreshPredictiveMetrics();
  }
  static getPredictiveHistoricalData(m: string) {
    return getPredictiveHistoricalData(m);
  }
  static calculatePredictions(d: any[]) {
    return calculatePredictions(d);
  }
  static calculateARIMAPredictions(d: any[], l: string) {
    return calculateARIMAPredictions(d, l);
  }
  static addMonths(m: string, n: number) {
    return addMonths(m, n);
  }
  static calculateSavingsRate(m?: string) {
    return calculateSavingsRate(m);
  }
  static calculateBudgetCompliance(a: string, t: number, m?: string) {
    return calculateBudgetCompliance(a, t, m);
  }
  static calculateFixedCostsRatio(m?: string) {
    return calculateFixedCostsRatio(m);
  }
  ...
  static showOutlierAnalysis(t: any) {
    return showOutlierAnalysis(t);
  }
  static detectOutliers(t: any[]) {
    return detectOutliers(t);
  }
  static createTransactionDetailsTable(c: any, w: number) {
    return createTransactionDetailsTable(c, w);
  }
  static updateTransactionTable() {
    return updateTransactionTable();
  }
  static createTimeFilter(c: any) {
    return createTimeFilter(c);
  }
  static createPraeskriptiveAnalytics(c: any) {
    return createPraeskriptiveAnalytics(c);
  }

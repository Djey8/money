...
export function createAnalyticsLevelSelector(container: any) {
  ...
  const levels = [
    {
      id: 'deskriptive',
      label: StatsComponent.translateService.instant('BI.deskriptive'),
      subtitle: StatsComponent.translateService.instant('BI.deskriptiveSubtitle'),
      short: StatsComponent.translateService.instant('BI.deskriptiveShort'),
    },
    {
      id: 'explorative',
      label: StatsComponent.translateService.instant('BI.explorative'),
      subtitle: StatsComponent.translateService.instant('BI.explorativeSubtitle'),
      short: StatsComponent.translateService.instant('BI.explorativeShort'),
    },
    {
      id: 'praediktive',
      label: StatsComponent.translateService.instant('BI.praediktive'),
      subtitle: StatsComponent.translateService.instant('BI.praediktiveSubtitle'),
      short: StatsComponent.translateService.instant('BI.praediktiveShort'),
    },
    {
      id: 'praeskriptive',
      label: StatsComponent.translateService.instant('BI.praeskriptive'),
      subtitle: StatsComponent.translateService.instant('BI.praeskriptiveSubtitle'),
      short: StatsComponent.translateService.instant('BI.praeskriptiveShort'),
    },
  ];
  ...
  if (isMobileLayout) {
    ...
    const currentLevel =
      levels.find((l) => l.id === StatsComponent.activeAnalyticsLevel) || levels[0];

    // Header showing selected level (clickable to expand)
    const headerDiv = selectorDiv
      .append('div')
    ...
    levels.forEach((level, index) => {
      if (level.id !== StatsComponent.activeAnalyticsLevel) {
        const option = dropdownDiv
          .append('div')
          .style('cursor', 'pointer')
          .style('padding', '12px')
          .style(
            'border-bottom',
            index < levels.length - 1 ? '1px solid var(--color-muted-light)' : 'none',
          )
          .style('background', 'var(--color-surface)')
...
export function createAnalyticsPlaceholder(container: any) {
  const placeholderDiv = container
    .append('div')
    .style('background', 'var(--color-surface)')
    .style('padding', '40px')
    .style('margin', '20px')
  ...
  placeholderDiv
    ...
    .text(() => {
      switch (StatsComponent.activeAnalyticsLevel) {
        case 'explorative':
          return 'Explorative Analytics – Interaktive Datenexploration';
        case 'praediktive':
          return 'Prädiktive Analytics – Einfache Prognosen';
        case 'praeskriptive':
          return 'Präskriptive Analytics – What-if-Szenarien';
        default:
          return 'Analytics Level';
      }
    });
...
export function createBIDashboard1_FinancialOverview() {
  ...
  // KPI Tiles Container
  const kpiContainer = dashboardDiv
    .append('div')
  ...
  if (hasMultipleMonths) {
    const viewModeDiv = dashboardDiv
      .append('div')
      .style('display', 'flex')
    ...
    viewModeDiv
      ...
      .style(
        'background',
        StatsComponent.showAverageView ? 'var(--color-primary)' : 'var(--color-text-muted)',
      )
      .style('color', 'white')
      .style('border', 'none')
      ...
      .html(
        StatsComponent.showAverageView
  ...
  if (filterType === 'month' && selectedMonth && selectedMonth !== 'all') {
    ...
    displayIncome =
      StatsComponent.showAverageView && monthCount > 1
        ? totalData.income / monthCount
        : totalData.income;
    displayExpenses =
      StatsComponent.showAverageView && monthCount > 1
        ? totalData.expenses / monthCount
        : totalData.expenses;
  ...
  const viewModeLabel =
    StatsComponent.showAverageView && isMultiMonth
      ? ` (${StatsComponent.translateService.instant('BI.average')})`
      : ` (${StatsComponent.translateService.instant('BI.total')})`;
  ...
  createKPITile(
    kpiContainer,
    `${StatsComponent.translateService.instant('BI.kpiTotalIncome')}${periodLabel}`,
    displayIncome,
  ...
  createKPITile(
    kpiContainer,
    `${StatsComponent.translateService.instant('BI.kpiTotalExpenses')}${periodLabel}`,
    displayExpenses,
    AppStateService.instance.currency,
    '#f44336',
  );
  ...
  createKPITile(
    kpiContainer,
    StatsComponent.translateService.instant('BI.kpiSavingsRate'),
    savingsRate,
    '%',
    savingsRate > 10 ? '#4caf50' : '#ff9800',
    StatsComponent.translateService.instant('BI.kpiSavingsRateGoal'),
  );

  // KPI 4: Fixed Costs Ratio (clickable - drills down to Dashboard 4)
  createKPITile(
    kpiContainer,
    StatsComponent.translateService.instant('BI.kpiFixedCostsRatio'),
    fixedCostsRatio,
    '%',
    fixedCostsRatio < 60 ? '#4caf50' : '#f44336',
  ...
  const chart1ButtonContainer = dashboardDiv
    .append('div')
    .style('display', 'flex')
  ...
  chart1ButtonContainer
    ...
    .html(
      StatsComponent.incomeVsExpensesChartMode === 'line'
        ? StatsComponent.translateService.instant('BI.chartModeBars')
        : StatsComponent.translateService.instant('BI.chartModeLines'),
    )
    .on('click', function () {
      StatsComponent.incomeVsExpensesChartMode =
        StatsComponent.incomeVsExpensesChartMode === 'line' ? 'bar' : 'line';
  ...
  const chart1Width =
    dataPointCount <= 1
      ? Math.min(400, baseChartWidth)
      : dataPointCount <= 3
        ? Math.min(600, baseChartWidth)
        : baseChartWidth;

  const chart1Container = dashboardDiv
    .append('div')
  ...
  const chart1Svg = chart1Container
    .append('svg')
  ...
  const chart2Container = dashboardDiv
    .append('div')
    .style('display', 'flex')
    .style('justify-content', 'center')
    .style('width', '100%');

  const chart2Svg = chart2Container
    .append('svg')
  ...
  const chart3Container = dashboardDiv
    .append('div')
    .style('cursor', 'pointer')
...
export function updateDashboard1ViewMode() {
  ...
  const formattedIncome = displayIncome.toLocaleString('de-DE', {
  ...
  const formattedExpenses = displayExpenses.toLocaleString('de-DE', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });
  kpiTiles
...
export function createBIDashboard2_AccountBudgetAnalysis() {
  ...
  const timeSeriesSvg = dashboardDiv
    .append('svg')
    .attr('width', chartWidth)
    .attr('height', 350)
    .style('max-width', '100%')
...
export function createBIDashboard3_CategoryAnalysis() {
  ...
  const chartWidth = width - 30; // Account for padding
  const topCategoriesSvg = dashboardDiv
    .append('svg')
    .attr('width', chartWidth)
    .attr('height', 400)
    .style('max-width', '100%')
  ...
  const timelineSvg = dashboardDiv
    .append('svg')
  ...
  const tableSection = dashboardDiv.append('div').attr('id', 'transaction-table-section');
...
export function createTimeFilter(container: any) {
  ...
  const months = Array.from(
    new Set(AppStateService.instance.allTransactions.map((t) => t.date.substring(0, 7))),
  ).sort();

  const years = Array.from(new Set(months.map((m) => m.substring(0, 4)))).sort();

  // Filter type selector
  const typeSelect = filterDiv
    .append('select')
    ...
    .on('change', function () {
      ...
      if (newFilterType === 'all') {
        ...
        StatsComponent.selectedMonth =
          years[years.length - 1] || new Date().getFullYear().toString();
  ...
  const selectorContainer = filterDiv.append('span').attr('id', 'selector-container');

  // Month selector (shown when filterType is "month")
  if (StatsComponent.filterType === 'month') {
    const monthSelect = selectorContainer
      .append('select')
      .style('padding', '8px 12px')
      .style('border', '1px solid var(--color-border)')
      .style('border-radius', '5px')
  ...
  if (StatsComponent.filterType === 'quarter') {
    ...
    const quarterSelect = selectorContainer
      .append('select')
      .style('padding', '8px 12px')
      .style('border', '1px solid var(--color-border)')
      .style('border-radius', '5px')
    ...
    if (
      !currentSelection ||
      !currentSelection.includes('-Q') ||
      !quarters.includes(currentSelection)
    ) {
  ...
  if (StatsComponent.filterType === 'halfyear') {
    ...
    const halfYearSelect = selectorContainer
      .append('select')
      .style('padding', '8px 12px')
      .style('border', '1px solid var(--color-border)')
      .style('border-radius', '5px')
    ...
    if (
      !currentSelection ||
      !currentSelection.includes('-H') ||
      !halfYears.includes(currentSelection)
    ) {
  ...
  if (StatsComponent.filterType === 'year') {
    const yearSelect = selectorContainer
      .append('select')
      .style('padding', '8px 12px')
      .style('border', '1px solid var(--color-border)')
      .style('border-radius', '5px')
...
export function createBIDashboard4_CostsAnalysis() {
  ...
  if (StatsComponent.dashboard4CostTypeFilter !== 'all') {
    const filterIndicator = container
      .append('div')
      .style('background', 'var(--color-surface)')
    ...
    filterIndicator
      ...
      .style('color', 'var(--color-text-secondary)')
      .text(
        StatsComponent.translateService.instant('BI.filterActive') +
          ': ' +
...
export function createDashboard4CostTypeChart(container: any, transactions: any[]) {
  ...
  const color = d3.scaleOrdinal().domain(['fixed', 'variable']).range(['#ff9800', '#1976d2']);
  ...
  arcs
    ...
    .attr('stroke', (d: any) => {
      return StatsComponent.dashboard4CostTypeFilter === d.data.type
        ? 'var(--color-text)'
        : 'white';
    })
  ...
  const legend = svg.append('g').attr('transform', `translate(${width / 2}, ${height + 20})`);

  data.forEach((d, i) => {
    const legendRow = legend.append('g').attr('transform', `translate(${i === 0 ? -120 : 20}, 0)`);
    ...
    legendText.append('tspan').attr('x', 20).attr('dy', 0).text(`${d.label}:`);

    const formattedAmount = d.amount.toLocaleString('de-DE', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    });
    legendText
...
export function createDashboard4CategoryTable(container: any, transactions: any[]) {
  ...
  const tableWrapper = tableSection
    .append('div')
  ...
  headers.forEach((header) => {
    ...
    if (header.column) {
      ...
      th.on('click', () => {
        if (StatsComponent.dashboard4SortColumn === header.column) {
          StatsComponent.dashboard4SortOrder =
            StatsComponent.dashboard4SortOrder === 'asc' ? 'desc' : 'asc';
        } else {
  ...
  categoryData.forEach((d, i) => {
    ...
    row
      ...
      .style(
        'font-weight',
        StatsComponent.dashboard4SelectedCategory === d.category ? 'bold' : 'normal',
      )
      .text(d.category);
    ...
    row.append('td').style('padding', '12px').text(d.count);

    row
      .append('td')
      .style('padding', '12px')
      .text(
        `${d.amount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}`,
      );
    ...
    const actionCell = row.append('td').style('padding', '12px');
...
export function createDashboard4TransactionTable(container: any, transactions: any[]) {
  ...
  tableSection
    ...
    .style('margin-bottom', '15px')
    .text(
      StatsComponent.translateService.instant('BI.transactionDetails') +
        `: ${StatsComponent.dashboard4SelectedCategory}`,
    );
  ...
  tableSection
    ...
    .style('margin-bottom', '15px')
    .text(
      `${categoryTransactions.length} ${StatsComponent.translateService.instant('BI.transactions')} | ` +
  ...
  categoryTransactions.forEach((t, i) => {
    ...
    row.append('td').style('padding', '12px').text(formattedDate);

    row.append('td').style('padding', '12px').text(t.account);
    ...
    row
      ...
      .style('font-weight', 'bold')
      .text(
        `${Math.abs(t.amount).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${AppStateService.instance.currency}`,
      );
...
export function createBIDashboardNavigation(container: any, activeDashboard: number) {
  ...
  dashboards.forEach((dash) => {
    navDiv
      ...
      .style(
        'background',
        dash.id === activeDashboard ? 'var(--color-primary)' : 'var(--color-surface)',
      )
 ...
 */
export function createIncomeVsExpensesBarChart(
  svg: any,
  data: any[],
  width: number,
  height: number,
  ...
  // Scales
  const x0 = d3.scaleBand().domain(dateStrings).range([0, chartWidth]).padding(0.2);
...
export function updateIncomeVsExpensesChart(filteredData: any[], chartWidth: number) {
  // Update button text
  d3.select('#income-vs-expenses-toggle-btn').html(
    StatsComponent.incomeVsExpensesChartMode === 'line'
      ? '📊 ' + StatsComponent.translateService.instant('BI.chartModeBars')
      : StatsComponent.translateService.instant('BI.chartModeLines'),
  );
...
export function createSavingsRateChart(svg: any, data: any[], width: number, height: number) {
  ...
  g.selectAll('.bar')
    ...
    .attr('fill', (d) =>
      d.savingsRate >= 10 ? '#4caf50' : d.savingsRate >= 0 ? '#ff9800' : '#f44336',
    )
    .attr('opacity', 0.85)
    .style('cursor', 'pointer')
    .on('mouseover', function () {
 ...
 */
export function createFixedVsVariableCostsChart(
  svg: any,
  width: number,
  height: number,
  isMultiMonth = false,
  ...
  const legend = g.append('g').attr('transform', `translate(0, -35)`);

  // Fixkosten (left-aligned at 0)
  const fixkostenLegend = legend.append('g').attr('transform', `translate(0, 0)`);
  ...
  const variableLegend = legend.append('g').attr('transform', `translate(${chartWidth - 150}, 0)`);
...
export function getAccountBalances(): {
  ...
  const accountData = accounts.map((account) => {
    ...
    const compliance = StatsComponent.calculateBudgetCompliance(
      account,
      targetPercent,
      StatsComponent.selectedMonth,
    );
...
export function createAccountBalanceTable(container: any, data: any[], width: number) {
  ...
  thead
    ...
    .selectAll('th')
    .data([
      StatsComponent.translateService.instant('BI.columnAccount'),
      StatsComponent.translateService.instant('BI.columnCurrentBalance'),
      StatsComponent.translateService.instant('BI.columnActualRate'),
      StatsComponent.translateService.instant('BI.columnTargetRate'),
      StatsComponent.translateService.instant('BI.columnCompliance'),
      StatsComponent.translateService.instant('BI.columnStatus'),
    ])
    .enter()
    .append('th')
    .style('padding', '12px')
  ...
  rows
    ...
    .text(
      (d) =>
        d.balance.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) +
        ' ' +
        AppStateService.instance.currency,
    );
  ...
  rows
    ...
    .style('font-weight', 'bold')
    .style('background', (d) =>
      d.compliance >= 80 ? '#4caf50' : d.compliance >= 50 ? '#ff9800' : '#f44336',
    )
    .style('color', 'white')
...
export function createMonthlyExpenseTable(container: any, width: number) {
  ...
  accounts.forEach((account, idx) => {
    ...
    row
      ...
      .style('background', 'var(--color-background)')
      .text(
        (accountTotals.get(account) || 0).toLocaleString('de-DE', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }) + ' €',
      );
...
export function createBudgetComplianceChart(svg: any, data: any[], width: number, height: number) {
  ...
  const x1 = d3.scaleBand().domain(['Ist', 'Ziel']).range([0, x0.bandwidth()]).padding(0.05);

  const y = d3.scaleLinear().domain([0, 100]).nice().range([chartHeight, 0]);

  // Draw bars
  const accountGroups = g
  ...
  accountGroups
    ...
    .attr('fill', (d) =>
      d.compliance >= 80 ? '#48b5c4' : d.compliance >= 50 ? '#ff9800' : '#f44336',
    );
  ...
  const legend = g.append('g').attr('transform', `translate(0, ${chartHeight + 40})`);

  const legendData = [
    {
      label: StatsComponent.translateService.instant('BI.legendActualCompliance'),
      color: '#48b5c4',
    },
...
export function createAccountDevelopmentChart(svg: any, width: number, height: number) {
  ...
  const timeSeriesData = Array.from(dailyMap.values()).sort(
  ...
  if (StatsComponent.filterType === 'month') {
    ...
    periodEndDate =
      timeSeriesData.length > 0
        ? new Date(timeSeriesData[timeSeriesData.length - 1].day)
        : new Date();
  }
  ...
  const y = d3
    ...
    .domain([
      d3.min(filledData, (d) =>
        Math.min(...accounts.map((acc) => d[acc + '_cumulative'])),
      ) as number,
      d3.max(filledData, (d) =>
        Math.max(...accounts.map((acc) => d[acc + '_cumulative'])),
      ) as number,
    ])
    .nice()
    .range([chartHeight, 0]);
...
export function createTopCategoriesChart(svg: any, width: number, height: number) {
  ...
  g.selectAll('.bar')
    ...
    .attr('fill', (d) => {
      ...
      const isSubscription = AppStateService.instance.allSubscriptions?.some(
        (sub) => sub.category === d.category,
      );
    ...
    .on('click', function (event, d) {
      ...
      g.selectAll('.bar')
        .attr('fill', (barData: any) => {
          ...
          const isSubscription = AppStateService.instance.allSubscriptions?.some(
            (sub) => sub.category === barData.category,
          );
          return isSubscription ? '#d3d3d3' : 'var(--color-primary)';
        })
        .attr('stroke', (barData: any) =>
          StatsComponent.selectedCategory === barData.category ? '#f57c00' : 'none',
        )
        .attr('stroke-width', (barData: any) =>
          StatsComponent.selectedCategory === barData.category ? 3 : 0,
        );
  ...
  g.selectAll('.label')
    ...
    .text(
      (d) =>
        d.amount.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) +
        ' ' +
        AppStateService.instance.currency +
        ' (' +
...
export function createCategoryTreemap(svg: any, width: number, height: number) {
  ...
  const data = {
    name: 'root',
    children: Array.from(categoryMap.entries()).map(([name, value]) => ({ name, value })),
  };
  ...
  cells
    ...
    .attr('opacity', 0.7)
    .attr('stroke', (d: any) =>
      StatsComponent.selectedCategory === d.data.name ? '#0d47a1' : 'var(--color-surface)',
    )
    ...
    .on('click', function (event, d: any) {
      ...
      svg
        .selectAll('rect')
        .attr('stroke', (rectData: any) =>
          StatsComponent.selectedCategory === rectData.data.name
            ? '#0d47a1'
            : 'var(--color-surface)',
        )
        .attr('stroke-width', (rectData: any) =>
          StatsComponent.selectedCategory === rectData.data.name ? 6 : 2,
        );
  ...
  cells
    ...
    .text((d: any) => {
      ...
      if (width > 60 && height > 40) {
        return (
          d.data.value.toLocaleString('de-DE', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }) +
          ' ' +
...
export function createCategoryTimelineChart(svg: any, width: number, height: number) {
  ...
  // Create scales
  const x = d3.scalePoint().domain(allMonths).range([0, chartWidth]);
  ...
  g.append('g')
    ...
    .call(
      d3
        .axisBottom(x)
        .tickValues(
          allMonths.filter((_, i) => i % Math.max(1, Math.floor(allMonths.length / 8)) === 0),
        ),
    )
    .selectAll('text')
    .style('text-anchor', 'end')
  ...
  if (width >= 600) {
    const legend = g.append('g').attr('transform', `translate(${chartWidth + 10}, 0)`);
 ...
 */
export function detectOutliers(transactions: any[]): {
  outliers: Map<number, { type: string; metadata?: any }>;
  recurringTransactions: Set<number>;
} {
  ...
  const categoryGroups = new Map<
    string,
    { amounts: number[]; indices: number[]; months: Set<string> }
  >();
...
export function createTransactionDetailsTable(container: any, width: number) {
  ...
  const { outliers: allOutliers, recurringTransactions: allRecurring } =
    detectOutliers(allTransactions);
  ...
  if (StatsComponent.selectedCategory) {
    filteredTransactions = filteredTransactions.filter(
      (t) => t.category === StatsComponent.selectedCategory,
    );
  }
  ...
  if (StatsComponent.selectedCategory) {
    ...
    filterInfo
      ...
      .style('color', 'var(--color-primary)')
      .text(
        `${StatsComponent.translateService.instant('BI.filteredByCategory')} ${StatsComponent.selectedCategory}`,
      );
  ...
  if (outlierCount > 0) {
    const outlierFilterDiv = controlsDiv
      .append('div')
      .style('display', 'flex')
      .style('align-items', 'center')
      .style('gap', '10px')
      ...
      .style(
        'background',
        StatsComponent.showOnlyOutliers ? 'var(--color-warning-bg)' : 'var(--color-background)',
      )
      .style('border-radius', '5px')
      .style(
        'border',
        `2px solid ${StatsComponent.showOnlyOutliers ? '#ff9800' : 'var(--color-border)'}`,
      );
    ...
    outlierFilterDiv
      ...
      .style('font-size', '12px')
      .text(
        StatsComponent.showOnlyOutliers
          ? StatsComponent.translateService.instant('BI.showAllButton')
          : StatsComponent.translateService.instant('BI.showOnlyOutliersButton'),
      )
      .on('click', function () {
  ...
  const thresholdDiv = controlsDiv
    .append('div')
    .style('display', 'flex')
    .style('align-items', 'center')
    .style('gap', '8px')
  ...
  const thresholdInput = thresholdDiv
    .append('input')
    .attr('type', 'number')
    .attr('min', '0.5')
    .attr('max', '3.0')
  ...
  // Sort transactions based on current sort settings
  const recentTransactions = filteredTransactions.slice().sort((a, b) => {
    let comparison = 0;
...
export function updateTransactionTable() {
  ...
  // Update treemap selection
  d3.selectAll('#transaction-table-section')
    .selectAll('rect')
    .each(function (d: any) {

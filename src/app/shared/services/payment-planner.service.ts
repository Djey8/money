...
export class PaymentPlannerService {
  ...
  generateBucketAllocationComment(allocations: BucketAllocation[]): string {
    return allocations.map((a) => `#bucket:${a.bucketTitle}:${a.amount.toFixed(2)}`).join(' ');
  }
  ...
  calculatePaymentPlan(config: {
    ...
    const periods = this.calculateNumberOfPeriods(
      config.startDate,
      config.targetDate,
      config.frequency,
    );
    ...
    const finalAmount =
      config.manualAmount !== undefined ? config.manualAmount : roundedCalculatedAmount;
  ...
  validateMultiplePlans(
    ...
    buckets.forEach((bucket) => {
      const fundingPlans = activePlans.filter(
        (plan) => plan.targetBucketIds.length === 0 || plan.targetBucketIds.includes(bucket.id),
      );

...
setTimeout(() =>
  import('../info-smile/info-smile.component').then(
...
export class InfoFireComponent extends BaseInfoComponent {
  ...
  get relatedTransactions() {
    ...
    const transactions = AppStateService.instance.allTransactions.filter((tx) => {
      ...
      if (tx.comment && fire.buckets?.length) {
        return fire.buckets.some((bucket) =>
  ...
  saveBucketEdit() {
    if (this.editingBucketIndex >= 0) {
      ...
      this.buckets[this.editingBucketIndex].target =
        Math.round(this.editingBucketTarget * 100) / 100;
      this.buckets[this.editingBucketIndex].amount =
        Math.round(this.editingBucketAmount * 100) / 100;
  ...
  updateFireEmergencie() {
    ...
    fire.completionDate =
      this.completionDateTextField || (this.phaseField === 'completed' ? now : '');
  ...
  advancePhase() {
    ...
    if (nextPhase !== InfoFireComponent.phase) {
      ...
      console.log(
        ...
        AppStateService.instance.allFireEmergencies.map((f) => ({
          title: f.title,
          phase: f.phase,
        })),
      );

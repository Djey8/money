...
// Routes shared by both editions. Never a fileReplacements target itself —
// app.routes.ts (Firebase) and app.routes.selfhosted.ts both import from
// here, so neither ends up importing a file that's also a replacement
// target for itself. See docs/adr/0004-edition-separation-mechanism.md.
export const baseRoutes: Routes = [
  ...
  },
  {
    ...
    loadComponent: () =>
      import('./docs/selfhosted/selfhosted-docs.component').then((m) => m.SelfhostedDocsComponent),
  },
  {
    path: 'community',
    loadComponent: () =>
  ...
  },
  {
    path: 'community/:id',
    loadComponent: () =>
      import('./community/thread/thread.component').then((m) => m.CommunityThreadComponent),
  },
  {
    path: 'authentication',
    loadComponent: () =>
